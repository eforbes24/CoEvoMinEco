...
// Set the Best Other for Coevolution (Eden Forbes)

void TSearch::SetBO(TVector<double> &bo)
{
	BO = bo;
}
...
void TSearch::DoCoSearch(int ResumeFlag, TVector<double> &BO)
{
	// Initialize search if necessary
	if (!SearchInitialized) InitializeSearch();
	// Make sure we have an evaluation function
	if (CoEvaluationFunction == NULL)
	{
		cerr << "Error: NULL evaluation function\n";
		exit(0);
	}
...
void TSearch::ExecuteCoSearch(TVector<double> &BO)
{
	DoCoSearch(0, BO);
}
...
double TSearch::CoEvaluateVector(TVector<double> &v, RandomState &rs, TVector<double> &BO)
{
	double perf = (*CoEvaluationFunction)(v, rs, BO);
	return (perf);
}
...
void *CoEvaluatePopulationRange(void *arg)
{
  PopRangeSpec *prs = (PopRangeSpec *)arg;
  TSearch *s = prs->search;
  for (int i = prs->start; i <= prs->end; i++)
    s->Perf[i] = s->CoEvaluateVector(s->Population[i], s->RandomStates[i], s->BO);
  pthread_exit(NULL);
}
...
void TSearch::CoEvaluatePopulation(int start, TVector<double> &BO)
{
#ifdef THREADED_SEARCH  // Evaluate the population in parallel
  // Create threads
  if (THREAD_COUNT > 1) {
    ...
    for (int i = 1; i <= THREAD_COUNT - 1; i++) {
      ...
      psrs[i-1].start = (i-1)*NumIndividuals + start;
      psrs[i-1].end = i*NumIndividuals + start - 1;
	  rc = pthread_create(&threads[i-1], NULL, CoEvaluatePopulationRange, (void *)&psrs[i-1]);
	  
      if (rc) {cerr << "Thread creation failed: " << rc << endl; exit(-1);}
    }
    // Evaluate the remaining individuals in the main thread
    for (int i = (THREAD_COUNT - 1)*NumIndividuals + start; i <= PopulationSize(); i++){
      Perf[i] = CoEvaluateVector(Population[i], RandomStates[i], BO);}
    // Wait for all other threads to complete
    int status;
    for (int i = 0; i <= THREAD_COUNT-2; i++)
      pthread_join(threads[i], (void **)&status);
...
void TSearch::ReproducePopulation(void)
{
	switch (RepMode) {
		...
		case CO_GENETIC_ALGORITHM: CoReproducePopulationGeneticAlgorithm(BO); break;
		default: cerr << "Invalid reproduction mode" << endl; exit(0);
	}
}

void TSearch::CoReproducePopulation(TVector<double> &BO)
{
	switch (RepMode) {
		case CO_GENETIC_ALGORITHM: CoReproducePopulationGeneticAlgorithm(BO); break;
		default: cerr << "Invalid reproduction mode" << endl; exit(0);
...
void TSearch::ReproducePopulationGeneticAlgorithm(void)
{
  ...
  // Evaluate the new population IN ENTIRETY
  CoEvaluatePopulation(1,BO);
//   if (ReEvalFlag) EvaluatePopulation();
//   else EvaluatePopulation(ElitePop+1);
}

void TSearch::CoReproducePopulationGeneticAlgorithm(TVector<double> &BO)
{
	int psize = PopulationSize();
